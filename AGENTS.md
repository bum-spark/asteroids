...
- Power-up "Triple shot": 25% drop from destroyed asteroids (pickup with ttl); collecting sets `ship.tripleTimer = POWERUP_DURATION` so `tryShoot()` fires 3 parallel bullets perpendicular to heading; HUD bar bottom-left. PowerUp has `type` ('velocidad' | 'triple').
